// Runtime i18n. The app is language-switchable via LangProvider + useLang().
// Server code (API routes, cron, server components) reads the chosen lang from
// the request's ?lang=... query param and uses the helpers below.

export type Lang = "en" | "es";
export const LANGS: readonly Lang[] = ["en", "es"] as const;

export const dict = {
  en: {
    appName: "Grammar",
    tagline: "Test your grammar agility",
    tabPlay: "Home",
    tabHistory: "History",
    tabYou: "You",
    statsLinkLabel: "Live stats",
    nerdosTagline: "Daily games for nerdos. Rewards for curious minds.",
    nerdosPickAGame: "Pick a game",
    grammarCardTitle: "Grammar",
    grammarCardBlurb:
      "Complete the sentence by choosing the correct word before time runs out.",
    mathCardTitle: "Math",
    mathCardBlurb:
      "Decide whether the math operation is correct or incorrect before time runs out.",
    cardLive: "Live",
    cardSoon: "Soon",
    backToPicker: "nerdos.fun",
    // Onboarding slides (picker home, first visit + "How it works" replay).
    obTitle1: "Daily games for nerdos",
    obBody1:
      "Two games, a new round every day: Grammar and Math. A run takes under a minute and plays straight from your wallet.",
    obTitle2: "One wrong answer ends the run",
    obBody2:
      "Answer fast — the timer gets shorter with every question. A single mistake ends it, and your best streak is your score.",
    obTitle3: "Every paid play is a ticket",
    obBody3:
      "Paid plays ($0.10) earn 1 to 3 tickets each, by score. When the day closes, one ticket is drawn and takes the whole prize in USDT — verifiable on-chain.",
    obTitle4: "Ready to play?",
    obBody4:
      "Pick Grammar or Math, keep a little USDT in your wallet, and go. A fresh prize starts every day.",
    obSkip: "Skip",
    obNext: "Next",
    obStart: "Let's go",
    obHowItWorks: "How it works",
    // Math chrome
    mathTitle: "Math",
    mathRule1: "You see an equation. Decide if the result shown is right or wrong.",
    mathRule2: "2.5 seconds at first — the timer gets shorter every question.",
    mathRule3: "One wrong answer = game over. Paid plays earn tickets for the daily prize draw.",
    mathReady: "I'M READY",
    mathScore: "SCORE",
    mathLevel: "LEVEL",
    mathFirstHint: "First one — take your time",
    // Shared between Grammar and Math game-over screens — same wording
    // works for both, so it lives here without a game prefix.
    timeUpHeadline: "⏰  Time's up",
    gameOverHeadline: "💥  Game over",
    clearedHeadline: "🏆  Cleared the deck",
    // Game-over answer reveal. "Tapped" (not "answered") because on a timeout
    // there was no answer at all, and the whole point of the line is to remind
    // the player which side their thumb actually hit.
    revealCorrectLabel: "Correct answer",
    revealPickedLabel: "You tapped",
    revealNoPick: "You ran out of time",
    retry: "RETRY",
    startErrorHeadline: "Couldn't start the run",
    startErrorFallback: "Could not start the run.",
    startErrorFooter: "Your payment is safe — the same tx is reused on retry.",
    sponsorEmptyState: "No sponsors yet — be the first to boost a prize.",
    // /stats page — keys flat (the page is a server component and reads
    // the Accept-Language header to pick a dict).
    statsHeading: "Stats",
    statsLiveRefresh: "Live · refresh to update",
    statsDbNotConfigured: "Database not configured.",
    statsSectionToday: "Today",
    statsSectionPlayers: "Players",
    statsSectionPlays: "Plays",
    statsSectionEconomy: "Economy",
    statsSectionOnchain: "On-chain",
    statsSectionSponsors: "Sponsors",
    statsSectionWebAnalytics: "Web analytics",
    statsCardRetention: "Retention",
    statsCardPlaysPerPlayer: "Plays per player",
    statsCardPlaysLast30: "Plays — last 30 days",
    statsCardPaidPlaysLast30: "Paid plays — last 30 days",
    statsChartBreakEven: "break-even",
    statsChartBreakEvenHint: "paid plays to cover the daily prize seeding",
    statsChartPerDay: "/day",
    statsChartSeeded: "prize seeding",
    statsChartNet: "net",
    statsCardByGame: "By game",
    statsCardTxByType: "Transactions by type",
    statsCardContracts: "Contracts",
    statsCardTopCountries: "Top countries (last 30 days)",
    statsCardFunnel: "Funnel (last 30 days)",
    statsCardDevices: "Devices",
    statsCardTrafficSources: "Top traffic sources",
    statsDau: "DAU",
    statsPlaysToday: "Plays today",
    statsNewSignups: "New email signups",
    statsPot: "Prize",
    statsOperatorGas: "Operator network fees",
    statsTopScore: "top score",
    statsNoPlaysYet: "no plays yet",
    statsAirdropsLeft: "airdrops left",
    statsPaid: "paid",
    statsFree: "free",
    statsPlayersTotal: "Total",
    statsWau: "WAU",
    statsMau: "MAU",
    statsPaidConversion: "Paid conversion",
    statsLast7Days: "last 7 days",
    statsLast30Days: "last 30 days",
    statsEverPaid: "ever paid",
    statsDistOne: "1 play",
    statsDistTwo: "2 plays",
    statsDistThreeToFive: "3-5 plays",
    statsDistSixToTen: "6-10 plays",
    statsDistElevenPlus: "11+ plays",
    statsRetentionCohort: "Cohort",
    statsRetentionReturned: "Returned",
    statsRetentionRate: "Rate",
    statsRetentionDay1to2: "Day 1 → Day 2",
    statsRetentionDay1to7: "Day 1 → Day 7",
    statsRetentionDay1to30: "Day 1 → Day 30",
    statsPlaysTotal: "Total plays",
    statsPlaysPaid: "Paid",
    statsPlaysFree: "Free",
    statsAvgScore: "Avg score",
    statsLangColGame: "Game",
    statsLangColPlays: "Plays",
    statsLangColPaid: "Paid",
    statsLangColPlayers: "Players",
    statsLangColRevenue: "Revenue",
    statsLangColPaidOut: "Paid out",
    statsRevenue: "Revenue",
    statsRevenueHint: "protocol fees",
    statsCardPnl: "P&L",
    statsPnlAllTime: "all time",
    statsPnlPre: "Before MiniPay",
    statsPnlPost: "Since MiniPay",
    statsPnlRevenue: "Revenue",
    statsPnlPrizeCost: "Prizes",
    statsPnlNet: "Net",
    statsPnlHint:
      "Revenue is the 20% cut of every paid entry. Prizes count only the part we fund ourselves — the other 80% of each entry stays in the pot, so players fund most of what the winner takes home.",
    statsPaidOut: "Paid out",
    statsToWinners: "to winners",
    statsBiggestPot: "Biggest prize",
    statsDaysClosed: "days closed",
    statsTreasury: "Treasury",
    statsRunwayDays: "d runway",
    statsTotalTx: "Total transactions",
    statsActiveAddresses: "active addresses",
    statsPlaysOnchain: "Plays on-chain",
    statsPlaysHitContract: "every play hits the contract",
    statsDaysOnchain: "Days on-chain",
    statsSinceFirstPlay: "since first play",
    statsUsdtIn: "USDT volume in",
    statsFromPaidEntries: "from paid entries",
    statsUsdtOut: "USDT volume out",
    statsOperatorTxs: "Operator txs",
    statsAirdrops: "airdrops",
    statsRolls: "rolls",
    statsContractGas: "Network fees spent",
    statsContractGasHint: "txs to contract",
    // MiniPay Stage-2 metrics (user-only slice + revert rate)
    statsUserGas: "Network fees paid by users",
    statsUserGasHint: "player txs",
    statsFailedTxRate: "Failed-tx rate",
    statsFailedTxRateHint: "reverts on player txs",
    statsTxColType: "Type",
    statsTxColCount: "Count",
    statsTxColShare: "Share",
    statsTxRowPlays: "Plays",
    statsTxRowRolls: "Roll-days",
    statsTxRowWelcomeAirdrops: "Welcome airdrops",
    statsTxRowSponsorAirdrops: "Sponsor airdrops",
    statsTxRowClaims: "Claims",
    statsContractPot: "Prize contract",
    statsContractOperator: "Operator",
    statsContractUsdt: "USDT (Celo)",
    statsSponsorBalance: "Balance",
    statsSponsorDailyPerGame: "Daily / game",
    statsSponsorPaidOut: "Paid out",
    statsSponsorRunway: "Runway",
    statsSponsorDaysLeft: "left",
    statsVisitors7d: "Visitors 7d",
    statsVisitors30d: "Visitors 30d",
    statsSessions: "Sessions",
    statsConnectRate: "Connect rate",
    statsOfVisitors: "of visitors",
    statsFunnelVisitors: "Visitors",
    statsFunnelConnected: "Wallet connected",
    statsFunnelStarted: "Started a play",
    statsFunnelFinished: "Finished a play",
    statsEntryFee: "Entry fee:",
    statsProtocolCut: "Protocol cut: 20%",
    statsPerPlay: "/play",
    prize: "Prize",
    todaysPot: "Today's prize",
    todaysPots: "Today's prizes",
    pickGame: "Pick a game",
    tapToSwitch: "Tap to switch",
    closesIn: "Closes in",
    potShare: "$0.08 feeds the prize",
    // Daily draw (since 2026-09-21): the pot is raffled among the day's
    // paid runs instead of going to the top score.
    dailyDraw: "Daily draw",
    drawTicketsToday: "tickets today",
    drawYours: "yours",
    drawNoTickets: "No tickets yet — the first paid play could take the whole pot.",
    drawTodayTitle: "In today's draw",
    // {n} is replaced with the game's point step at render time.
    drawRule: "1 ticket per paid play · +1 every {n} pts · max 3 per play, unlimited plays",
    drawAnnounceTitle: "New: the pot is now a daily draw",
    drawAnnounceBody:
      "Top score no longer takes the pot: it's drawn among the day's paid plays — every play adds tickets. The full rule is on each pot card.",
    drawAnnounceCta: "Got it",
    drawTicketInDraw: "Your ticket is in today's draw",
    // History: step-by-step proof of how a past draw picked its winner.
    // {k}/{n}/{g} are replaced at render time.
    drawHowPicked: "How was the winner picked?",
    drawWinnerOdds: "{k} of {n} tickets 🎟",
    drawCarriedOver: "Nobody entered — the pot carried over",
    drawStep1: "Entries closed at midnight UTC. The first Celo block after that decided the draw:",
    drawBlock: "Block",
    drawTime: "Time",
    drawBlockHash: "Block hash",
    drawStep2: "Nobody can know that hash before it's mined — not even us. Mixed with the game number ({g}) it gives the seed:",
    drawSeed: "Seed",
    drawStep3: "Seed ÷ {n} tickets → the remainder + 1 is the winning ticket:",
    drawWinningTicket: "Winning ticket",
    drawStep4: "Who held each ticket (paid plays, in run order):",
    drawVerify: "Check it yourself",
    drawOnCeloscan: "Block on Celoscan",
    drawOnchainRecord: "On-chain result",
    drawRawData: "Full data (JSON)",
    // Game-over ticket feedback. {k} = tickets this run earned, {n} = the
    // score that would have earned one more.
    gameOverTicketOne: "This play holds 1 ticket in today's draw",
    gameOverTicketsMany: "This play holds {k} tickets in today's draw",
    gameOverNextTicket: "Reach {n} pts for an extra ticket",
    gameOverFreeNudge:
      "The pot is drawn among paid plays — your next paid play enters today's draw",
    leaderboardPrestige: "Top scores are for glory — the pot is decided by the draw",
    leaderboard: "Leaderboard",
    todaysLeaderboard: "Today's leaderboard",
    finalStandings: "Final standings",
    noPlaysYet: "No plays yet today — be the first.",
    play: "Play",
    playFree: "Play free",
    playPaid: "Play",
    freePlayUsed: "Free play used",
    freeAgainIn: "Free again in",
    score: "Score",
    timeLeft: "Time",
    getReady: "Get ready",
    tapCorrect: "Tap the correct word",
    rulesTime: "5 seconds per question",
    rulesMiss: "One mistake ends the game",
    imReady: "I'm ready",
    rulesHint: "5s per question · paid plays earn draw tickets",
    // Tutorial Q1 (first run ever)
    firstPlayBadge: "No clock",
    firstPlayHint: "First question — tap when you're ready. The 5s clock starts after.",
    signInWithEmail: "Sign in with email",
    useYourOwnWallet: "Use your own wallet",
    go: "GO!",
    gameOver: "Game over",
    yourScore: "Your score",
    yourRank: "Your rank",
    playAgain: "Play again",
    backToLobby: "Back to home",
    share: "Share",
    youHaveUnclaimed: "Unclaimed wins",
    claimAll: "Claim all",
    claim: "Claim",
    claimed: "Claimed",
    pastGames: "Past games",
    noHistoryYet: "No past games yet — play today.",
    stats: "Stats",
    gamesPlayed: "Games played",
    wins: "Wins",
    totalEarned: "Total earned",
    winner: "Winner",
    noWinner: "No winner",
    connecting: "Connecting…",
    connect: "Connect wallet",
    // WalletSection
    wallet: "Wallet",
    copied: "copied ✓",
    send: "Send →",
    sendToAnotherWallet: "Send to another wallet",
    sendModalHint:
      "Send {token} to any address on the Celo network — a friend, an exchange, or your own wallet.",
    sendNetworkNote:
      "This transfer happens on the Celo network. Make sure the recipient is also on Celo.",
    topUp: "Top up →",
    available: "Available",
    toAddress: "To address",
    selfAddressWarning: "That’s your own address.",
    amount: "Amount",
    max: "Max",
    sending: "Sending…",
    sendToken: "Send",
    close: "Close",
    descUSDT: "Digital US Dollar",
    descCOPm: "Digital Colombian Peso",
    descCELO: "Used for transaction fees",
    // NeedFundsModal — MiniPay branch (single-CTA → add_cash deeplink).
    // The hint explicitly names swap-from-USDC/USDm because nerdos.fun
    // only accepts USDT — MiniPay's listing rules require a graceful
    // single-token explainer so a user holding USDC/USDm doesn't bounce
    // off thinking the app is broken. The add_cash screen handles both
    // swap and external deposit; we don't have to branch on user
    // balances client-side.
    mpDepositTitle: "Add USDT to play",
    mpDepositHint:
      "Top up in MiniPay — deposit USDT or swap from USDC or USDm.",
    mpDepositCta: "Deposit",
    // NeedFundsModal
    notEnoughUSDT: "Not enough USDT",
    notEnoughCELO: "Not enough CELO",
    blurbUSDT: "USDT pays your entry and funds the prize.",
    blurbCELO: "CELO is the tiny fee every transaction on Celo costs.",
    hintTopUp: "Top up {token} on the Celo network to the wallet below, then try again.",
    youHave: "You have",
    youNeed: "You need",
    bridgeTo: "Bridge → {token} on Celo",
    fromOtherChains: "From Ethereum, Base, Polygon… via Squid",
    swapCeloUsdt: "Swap CELO → USDT",
    swapUsdtCelo: "Swap USDT → CELO",
    onUniswap: "On Uniswap (Celo)",
    orWithdrawFromExchange:
      "Or withdraw {token} via the Celo network from any exchange (Binance, Coinbase, OKX…).",
    // /you empty state
    youSignInTitle: "Sign in to see your stats",
    youSignInBlurb:
      "Your wins, unclaimed rewards and wallet balances appear here once you connect.",
    // Shown inside MiniPay while the auto-connect is settling. MiniPay
    // listings ban Connect-Wallet buttons, so the email + RainbowKit
    // sign-in pair gets replaced by this single status line.
    miniPayConnecting: "Connecting to MiniPay…",
    // Support — rendered in the /you footer. Required as an in-app
    // contact path by MiniPay's listing rules. Channel-agnostic label
    // so we can swap the link without touching copy.
    supportLabel: "Help & support",
    supportTitle: "Help & support",
    supportBody:
      "We answer questions in our Telegram group — usually within a few hours.",
    supportOpenTelegram: "Open Telegram",
    supportCopyLink: "Copy link instead",
    // Legal links — rendered in the /you footer. Short labels so the row
    // doesn't crowd the disconnect button.
    termsLink: "Terms",
    privacyLink: "Privacy",
    // Disconnect — labelled "log out" because the click runs
    // privyLogout(), ending the Privy session entirely, not just
    // dropping a wagmi wallet connection.
    disconnectWallet: "Log out",
    disconnectHint: "Sign out to switch to a different account.",
    // Play stages (button label while a tx is in flight)
    stageSwitching: "Switching network…",
    stageApproving: "Approving USDT…",
    stageSigning: "Starting play…",
    stagePaying: "Paying $0.10…",
    stageStarting: "Starting…",
    // Resume banner
    resumeOne: "You have a play ready to resume",
    resumeMany: "You have {n} plays ready to resume",
    resumeTapHint: "tap to resume",
    resumeMoreAfter: "+{n} more after",
    resume: "Resume →",
    // PotCard
    extraPrize: "Extra prize",
    sponsoredBy: "Sponsored by",
    sponsorThisPot: "Sponsor this prize →",
    // Unclaimed banner
    tapToClaim: "tap to claim",
    // Claim list
    claimingStatus: "Claiming…",
    noPendingWins: "🎯  No pending wins. Go play.",
    readyBadge: "ready",
    youTag: "you",
    // Sponsor page CTA
    wantToSponsor: "Want to sponsor with your own token?",
    sponsorCtaBlurb:
      "Communities, DAOs and brands can fund bonus rewards on top of the daily prize — any Celo token, any amount, tagged with your name on the prize card.",
    talkToCamilo: "Talk to @camilosaka on Telegram →",
    // OR divider
    or: "OR",
    // Wallet section — explanatory copy for non-crypto users
    walletSubtitle: "Your money on the Celo network",
    purposeUSDT: "Used to enter paid games. Winnings are paid out in USDT.",
    purposeCOPm: "A bonus some sponsors may award, like Celo Colombia.",
    purposeCELO: "Covers the tiny fee every on-chain action costs.",
    addMoney: "Add money",
    addressHint:
      "This address is like your bank account number on Celo. Share it to receive USDT, CELO or COPm.",
    networkWarning:
      "This app only shows tokens on the Celo network.",
    // NeedFundsModal rework — three separated options
    addTokenTitle: "Add {token} to your wallet",
    receiveTitle: "Someone sends it to you",
    receiveHint:
      "This address is like your bank account number on Celo. Share it with whoever is sending you {token}.",
    bridgeTitle: "Bridge from another chain",
    bridgeHint:
      "If you already have stablecoins on Ethereum, Base, Polygon or another chain, move some of them over to Celo.",
    swapTitle: "Swap on Celo",
    swapHintUSDT: "If you already have CELO in your wallet, convert some into USDT here.",
    swapHintCELO: "If you already have USDT in your wallet, convert some into CELO here.",
    copyAddress: "Copy address",
  },
  es: {
    appName: "Gramática",
    tagline: "Pon a prueba tu agilidad en la gramática",
    tabPlay: "Inicio",
    tabHistory: "Historial",
    tabYou: "Tú",
    statsLinkLabel: "Estadísticas en vivo",
    nerdosTagline: "Juegos diarios para nerdos. Premios para mentes curiosas.",
    nerdosPickAGame: "Elige un juego",
    grammarCardTitle: "Gramática",
    grammarCardBlurb:
      "Completa la frase eligiendo la palabra correcta antes de que se acabe el tiempo.",
    mathCardTitle: "Matemáticas",
    mathCardBlurb:
      "Decide si la operación matemática es correcta o incorrecta antes de que se acabe el tiempo.",
    cardLive: "En vivo",
    cardSoon: "Pronto",
    backToPicker: "nerdos.fun",
    obTitle1: "Juegos diarios para nerdos",
    obBody1:
      "Dos juegos, una ronda nueva cada día: Gramática y Matemáticas. Una partida toma menos de un minuto y se juega directo desde tu wallet.",
    obTitle2: "Un error y se acaba",
    obBody2:
      "Responde rápido — el tiempo se acorta con cada pregunta. Un solo error termina la partida, y tu mejor racha es tu puntaje.",
    obTitle3: "Cada jugada pagada es un ticket",
    obBody3:
      "Las jugadas pagadas ($0.10) ganan de 1 a 3 tickets según tu puntaje. Al cierre del día se sortea un ticket y se lleva todo el premio en USDT — verificable on-chain.",
    obTitle4: "¿Listo para jugar?",
    obBody4:
      "Elige Gramática o Matemáticas, ten un poco de USDT en tu wallet y arranca. Cada día empieza un premio nuevo.",
    obSkip: "Saltar",
    obNext: "Siguiente",
    obStart: "Vamos",
    obHowItWorks: "Cómo funciona",
    mathTitle: "Matemáticas",
    mathRule1: "Ves una ecuación. Decide si el resultado mostrado es correcto o incorrecto.",
    mathRule2: "2.5 segundos al inicio — el reloj baja cada pregunta.",
    mathRule3: "Una respuesta mala = se acaba. Las jugadas pagadas ganan tickets para el sorteo del premio.",
    mathReady: "ESTOY LISTO",
    mathScore: "PUNTAJE",
    mathLevel: "NIVEL",
    mathFirstHint: "Primera — sin reloj, tómate tu tiempo",
    timeUpHeadline: "⏰  Se acabó el tiempo",
    gameOverHeadline: "💥  Fin del juego",
    clearedHeadline: "🏆  ¡Mazo completo!",
    revealCorrectLabel: "Respuesta correcta",
    revealPickedLabel: "Tocaste",
    revealNoPick: "Se te acabó el tiempo",
    retry: "REINTENTAR",
    startErrorHeadline: "No pudimos empezar la jugada",
    startErrorFallback: "No se pudo empezar la jugada.",
    startErrorFooter: "Tu pago está a salvo — se reutiliza la misma tx al reintentar.",
    sponsorEmptyState: "Aún no hay sponsors — sé el primero en subir un premio.",
    statsHeading: "Estadísticas",
    statsLiveRefresh: "En vivo · recarga para actualizar",
    statsDbNotConfigured: "Base de datos no configurada.",
    statsSectionToday: "Hoy",
    statsSectionPlayers: "Jugadores",
    statsSectionPlays: "Jugadas",
    statsSectionEconomy: "Economía",
    statsSectionOnchain: "On-chain",
    statsSectionSponsors: "Sponsors",
    statsSectionWebAnalytics: "Analítica web",
    statsCardRetention: "Retención",
    statsCardPlaysPerPlayer: "Jugadas por jugador",
    statsCardPlaysLast30: "Jugadas — últimos 30 días",
    statsCardPaidPlaysLast30: "Jugadas pagas — últimos 30 días",
    statsChartBreakEven: "equilibrio",
    statsChartBreakEvenHint: "jugadas pagas para cubrir el premio diario",
    statsChartPerDay: "/día",
    statsChartSeeded: "premios sembrados",
    statsChartNet: "neto",
    statsCardByGame: "Por juego",
    statsCardTxByType: "Transacciones por tipo",
    statsCardContracts: "Contratos",
    statsCardTopCountries: "Top países (últimos 30 días)",
    statsCardFunnel: "Embudo (últimos 30 días)",
    statsCardDevices: "Dispositivos",
    statsCardTrafficSources: "Top fuentes de tráfico",
    statsDau: "DAU",
    statsPlaysToday: "Jugadas hoy",
    statsNewSignups: "Nuevos registros por email",
    statsPot: "Premio",
    statsOperatorGas: "Tarifas de red del operador",
    statsTopScore: "mejor puntaje",
    statsNoPlaysYet: "sin jugadas",
    statsAirdropsLeft: "airdrops restantes",
    statsPaid: "pagas",
    statsFree: "gratis",
    statsPlayersTotal: "Total",
    statsWau: "WAU",
    statsMau: "MAU",
    statsPaidConversion: "Conversión a pago",
    statsLast7Days: "últimos 7 días",
    statsLast30Days: "últimos 30 días",
    statsEverPaid: "ha pagado alguna vez",
    statsDistOne: "1 jugada",
    statsDistTwo: "2 jugadas",
    statsDistThreeToFive: "3-5 jugadas",
    statsDistSixToTen: "6-10 jugadas",
    statsDistElevenPlus: "11+ jugadas",
    statsRetentionCohort: "Cohorte",
    statsRetentionReturned: "Volvieron",
    statsRetentionRate: "Tasa",
    statsRetentionDay1to2: "Día 1 → Día 2",
    statsRetentionDay1to7: "Día 1 → Día 7",
    statsRetentionDay1to30: "Día 1 → Día 30",
    statsPlaysTotal: "Jugadas totales",
    statsPlaysPaid: "Pagas",
    statsPlaysFree: "Gratis",
    statsAvgScore: "Puntaje prom.",
    statsLangColGame: "Juego",
    statsLangColPlays: "Jugadas",
    statsLangColPaid: "Pagas",
    statsLangColPlayers: "Jugadores",
    statsLangColRevenue: "Ingresos",
    statsLangColPaidOut: "Pagado",
    statsRevenue: "Ingresos",
    statsRevenueHint: "comisiones del protocolo",
    statsCardPnl: "P&L",
    statsPnlAllTime: "histórico",
    statsPnlPre: "Antes de MiniPay",
    statsPnlPost: "Desde MiniPay",
    statsPnlRevenue: "Revenue",
    statsPnlPrizeCost: "Premios",
    statsPnlNet: "Neto",
    statsPnlHint:
      "El revenue es la comisión del 20 % de cada partida paga. En premios contamos solo la parte que ponemos nosotros — el otro 80 % de cada entrada queda en el pozo, así que los jugadores financian la mayor parte de lo que se lleva el ganador.",
    statsPaidOut: "Pagado",
    statsToWinners: "a ganadores",
    statsBiggestPot: "Premio más alto",
    statsDaysClosed: "días cerrados",
    statsTreasury: "Tesoro",
    statsRunwayDays: "d de runway",
    statsTotalTx: "Transacciones totales",
    statsActiveAddresses: "direcciones activas",
    statsPlaysOnchain: "Jugadas on-chain",
    statsPlaysHitContract: "cada jugada toca el contrato",
    statsDaysOnchain: "Días on-chain",
    statsSinceFirstPlay: "desde la primera jugada",
    statsUsdtIn: "Volumen USDT entrada",
    statsFromPaidEntries: "de entradas pagas",
    statsUsdtOut: "Volumen USDT salida",
    statsOperatorTxs: "Tx del operador",
    statsAirdrops: "airdrops",
    statsRolls: "rolls",
    statsContractGas: "Tarifas de red gastadas",
    statsContractGasHint: "tx al contrato",
    statsUserGas: "Tarifas de red pagadas por jugadores",
    statsUserGasHint: "tx de jugadores",
    statsFailedTxRate: "Tasa de tx fallidas",
    statsFailedTxRateHint: "reverts en tx de jugadores",
    statsTxColType: "Tipo",
    statsTxColCount: "Cantidad",
    statsTxColShare: "Porcentaje",
    statsTxRowPlays: "Jugadas",
    statsTxRowRolls: "Roll-days",
    statsTxRowWelcomeAirdrops: "Airdrops de bienvenida",
    statsTxRowSponsorAirdrops: "Airdrops de sponsor",
    statsTxRowClaims: "Reclamos",
    statsContractPot: "Contrato de premios",
    statsContractOperator: "Operador",
    statsContractUsdt: "USDT (Celo)",
    statsSponsorBalance: "Balance",
    statsSponsorDailyPerGame: "Diario / juego",
    statsSponsorPaidOut: "Pagado",
    statsSponsorRunway: "Runway",
    statsSponsorDaysLeft: "restantes",
    statsVisitors7d: "Visitantes 7d",
    statsVisitors30d: "Visitantes 30d",
    statsSessions: "Sesiones",
    statsConnectRate: "Tasa de conexión",
    statsOfVisitors: "de visitantes",
    statsFunnelVisitors: "Visitantes",
    statsFunnelConnected: "Wallet conectada",
    statsFunnelStarted: "Inició una jugada",
    statsFunnelFinished: "Terminó una jugada",
    statsEntryFee: "Entrada:",
    statsProtocolCut: "Comisión del protocolo: 20%",
    statsPerPlay: "/jugada",
    prize: "Premio",
    todaysPot: "Premio de hoy",
    todaysPots: "Premios de hoy",
    pickGame: "Elige un juego",
    tapToSwitch: "Toca para cambiar",
    closesIn: "Cierra en",
    potShare: "$0.08 alimenta el premio",
    // Sorteo diario (desde 2026-09-21): el pote se sortea entre las jugadas
    // pagadas del día en vez de ir al mejor puntaje.
    dailyDraw: "Sorteo del día",
    drawTicketsToday: "tickets hoy",
    drawYours: "tuyos",
    drawNoTickets: "Aún no hay tickets — la primera jugada pagada puede llevarse todo el pote.",
    drawTodayTitle: "En el sorteo de hoy",
    // {n} se reemplaza con el paso de puntos del juego al renderizar.
    drawRule: "1 ticket por jugada pagada · +1 cada {n} pts · máx. 3 por jugada, sin límite de jugadas",
    drawAnnounceTitle: "Nuevo: el pote ahora se sortea",
    drawAnnounceBody:
      "El mejor puntaje ya no se lleva el pote: se sortea entre las jugadas pagadas del día — cada jugada suma tickets. La regla completa está en cada tarjeta.",
    drawAnnounceCta: "Entendido",
    drawTicketInDraw: "Tu ticket está en el sorteo de hoy",
    // Historial: prueba paso a paso de cómo un sorteo eligió al ganador.
    drawHowPicked: "¿Cómo se eligió al ganador?",
    drawWinnerOdds: "{k} de {n} tickets 🎟",
    drawCarriedOver: "Nadie entró — el pote pasó al día siguiente",
    drawStep1: "Las entradas cerraron a medianoche UTC. El primer bloque de Celo después de esa hora decidió el sorteo:",
    drawBlock: "Bloque",
    drawTime: "Hora",
    drawBlockHash: "Hash del bloque",
    drawStep2: "Nadie puede saber ese hash antes de que se mine el bloque — ni nosotros. Combinado con el número del juego ({g}) da la semilla:",
    drawSeed: "Semilla",
    drawStep3: "Semilla ÷ {n} tickets → el residuo + 1 es el ticket ganador:",
    drawWinningTicket: "Ticket ganador",
    drawStep4: "Quién tenía cada ticket (jugadas pagadas, en orden):",
    drawVerify: "Verifícalo tú mismo",
    drawOnCeloscan: "Bloque en Celoscan",
    drawOnchainRecord: "Resultado on-chain",
    drawRawData: "Datos completos (JSON)",
    // Feedback de tickets en el game-over. {k} = tickets de esta jugada,
    // {n} = puntaje que habría dado uno más.
    gameOverTicketOne: "Esta jugada tiene 1 ticket en el sorteo de hoy",
    gameOverTicketsMany: "Esta jugada tiene {k} tickets en el sorteo de hoy",
    gameOverNextTicket: "Llega a {n} pts y ganas un ticket extra",
    gameOverFreeNudge:
      "El pote se sortea entre jugadas pagadas — tu próxima jugada pagada entra al sorteo de hoy",
    leaderboardPrestige: "La tabla es por gloria — el pote lo decide el sorteo",
    leaderboard: "Tabla",
    todaysLeaderboard: "Tabla de hoy",
    finalStandings: "Resultado final",
    noPlaysYet: "Aún nadie ha jugado hoy — sé el primero.",
    play: "Jugar",
    playFree: "Jugada gratis",
    playPaid: "Jugar",
    freePlayUsed: "Gratis usada",
    freeAgainIn: "Gratis en",
    score: "Puntaje",
    timeLeft: "Tiempo",
    getReady: "Prepárate",
    tapCorrect: "Toca la palabra correcta",
    rulesTime: "5 segundos por pregunta",
    rulesMiss: "Un error y se acaba",
    imReady: "Estoy listo",
    rulesHint: "5s por pregunta · las pagadas ganan tickets",
    // Tutorial Q1 (first run ever)
    firstPlayBadge: "Sin reloj",
    firstPlayHint: "Primera pregunta — toca cuando estés listo. El reloj de 5s arranca después.",
    signInWithEmail: "Entrar con correo",
    useYourOwnWallet: "Usar mi propia wallet",
    go: "¡YA!",
    gameOver: "Fin del juego",
    yourScore: "Tu puntaje",
    yourRank: "Tu posición",
    playAgain: "Jugar otra",
    backToLobby: "Volver al inicio",
    share: "Compartir",
    youHaveUnclaimed: "Premios sin reclamar",
    claimAll: "Reclamar todo",
    claim: "Reclamar",
    claimed: "Reclamado",
    pastGames: "Juegos anteriores",
    noHistoryYet: "Todavía no hay historial — juega hoy.",
    stats: "Estadísticas",
    gamesPlayed: "Partidas",
    wins: "Victorias",
    totalEarned: "Total ganado",
    winner: "Ganador",
    noWinner: "Sin ganador",
    connecting: "Conectando…",
    connect: "Conectar wallet",
    // WalletSection
    wallet: "Cartera",
    copied: "copiado ✓",
    send: "Enviar →",
    sendToAnotherWallet: "Enviar a otra wallet",
    sendModalHint:
      "Envía {token} a cualquier dirección en la red Celo — un amigo, un exchange o tu propia wallet.",
    sendNetworkNote:
      "Esta transferencia se hace en la red Celo. Asegúrate de que el destinatario también esté en Celo.",
    topUp: "Recargar →",
    available: "Disponible",
    toAddress: "Dirección destino",
    selfAddressWarning: "Esa es tu propia dirección.",
    amount: "Cantidad",
    max: "Máx",
    sending: "Enviando…",
    sendToken: "Enviar",
    close: "Cerrar",
    descUSDT: "Dólar digital",
    descCOPm: "Peso colombiano digital",
    descCELO: "Usado para pagar tarifas de red",
    // NeedFundsModal — MiniPay branch (single-token explainer baked in)
    mpDepositTitle: "Agrega USDT para jugar",
    mpDepositHint:
      "Recarga en MiniPay — deposita USDT o cámbialo desde USDC o USDm.",
    mpDepositCta: "Depositar",
    // NeedFundsModal
    notEnoughUSDT: "No tienes USDT suficiente",
    notEnoughCELO: "No tienes CELO suficiente",
    blurbUSDT: "El USDT paga tu entrada y alimenta el premio.",
    blurbCELO: "CELO es el pequeño fee que cuesta cada transacción en Celo.",
    hintTopUp:
      "Recarga {token} por la red Celo a la wallet de abajo y vuelve a intentar.",
    youHave: "Tienes",
    youNeed: "Necesitas",
    bridgeTo: "Bridge → {token} en Celo",
    fromOtherChains: "Desde Ethereum, Base, Polygon… vía Squid",
    swapCeloUsdt: "Cambiar CELO → USDT",
    swapUsdtCelo: "Cambiar USDT → CELO",
    onUniswap: "En Uniswap (Celo)",
    orWithdrawFromExchange:
      "O retira {token} por la red Celo desde cualquier exchange (Binance, Coinbase, OKX…).",
    // /you empty state
    youSignInTitle: "Entra para ver tus stats",
    youSignInBlurb:
      "Tus victorias, premios pendientes y saldos aparecen acá cuando te conectes.",
    miniPayConnecting: "Conectando con MiniPay…",
    supportLabel: "Ayuda y soporte",
    supportTitle: "Ayuda y soporte",
    supportBody:
      "Respondemos tus preguntas en nuestro grupo de Telegram — normalmente en pocas horas.",
    supportOpenTelegram: "Abrir Telegram",
    supportCopyLink: "Mejor copiar el link",
    termsLink: "Términos",
    privacyLink: "Privacidad",
    // Disconnect — see EN note above
    disconnectWallet: "Cerrar sesión",
    disconnectHint: "Cierra sesión para cambiar de cuenta.",
    // Play stages (button label while a tx is in flight)
    stageSwitching: "Cambiando red…",
    stageApproving: "Autorizando USDT…",
    stageSigning: "Iniciando jugada…",
    stagePaying: "Pagando $0.10…",
    stageStarting: "Iniciando…",
    // Resume banner
    resumeOne: "Tienes una jugada lista para retomar",
    resumeMany: "Tienes {n} jugadas listas para retomar",
    resumeTapHint: "toca para retomar",
    resumeMoreAfter: "+{n} más después",
    resume: "Retomar →",
    // PotCard
    extraPrize: "Premio extra",
    sponsoredBy: "Patrocinado por",
    sponsorThisPot: "Patrocina este premio →",
    // Unclaimed banner
    tapToClaim: "toca para reclamar",
    // Claim list
    claimingStatus: "Reclamando…",
    noPendingWins: "🎯  No hay premios pendientes. A jugar.",
    readyBadge: "listo",
    youTag: "tú",
    // Sponsor page CTA
    wantToSponsor: "¿Quieres patrocinar con tu propio token?",
    sponsorCtaBlurb:
      "Comunidades, DAOs y marcas pueden fondear bonos adicionales encima del premio diario — cualquier token en Celo, cualquier monto, con tu nombre en la tarjeta del premio.",
    talkToCamilo: "Habla con @camilosaka en Telegram →",
    // OR divider
    or: "O",
    // Wallet section — copy explicativo para no-crypto
    walletSubtitle: "Tu plata en la red Celo",
    purposeUSDT: "Se usa para entrar a juegos pagos. Las victorias se pagan en USDT.",
    purposeCOPm: "Un bono que pueden premiar sponsors, como Celo Colombia.",
    purposeCELO: "Cubre el fee mínimo que cuesta cada acción on-chain.",
    addMoney: "Agregar plata",
    addressHint:
      "Esta dirección funciona como tu número de cuenta en la red Celo. Compártela para recibir USDT, CELO o COPm.",
    networkWarning:
      "Esta app solo muestra tokens en la red Celo.",
    // NeedFundsModal rework
    addTokenTitle: "Agregar {token} a tu wallet",
    receiveTitle: "Que alguien te la envíe",
    receiveHint:
      "Esta dirección funciona como un número de cuenta en Celo. Compártela con quien te va a enviar {token}.",
    bridgeTitle: "Bridge desde otra cadena",
    bridgeHint:
      "Si ya tienes stablecoins en Ethereum, Base, Polygon u otra cadena, mueve algo a Celo.",
    swapTitle: "Cambiar en Celo",
    swapHintUSDT: "Si ya tienes CELO en tu wallet, convierte una parte a USDT acá.",
    swapHintCELO: "Si ya tienes USDT en tu wallet, convierte una parte a CELO acá.",
    copyAddress: "Copiar dirección",
  },
} as const;

export type Strings = (typeof dict)[Lang];

/** Narrow any string to a valid Lang, defaulting to "en". */
export function validateLang(s: string | null | undefined): Lang {
  return s === "es" ? "es" : "en";
}

/** Contract gameId for a given language (matches the initGame calls). */
export function gameIdFor(lang: Lang): 1 | 2 {
  return lang === "en" ? 1 : 2;
}

/** Tiny template-style interpolator for dict strings with `{key}` slots. */
export function tpl(
  str: string,
  vars: Record<string, string | number>,
): string {
  return str.replace(/\{(\w+)\}/g, (_, k) =>
    vars[k] !== undefined ? String(vars[k]) : `{${k}}`,
  );
}
